<script lang="ts">
	...
	$: cost = tokenInfo.pricing
		? (usage.input_tokens * tokenInfo.pricing.input +
...
<div class="mt-4 text-sm">
	...
	>
		{#if tokenInfo.pricing}
			<span
				>Last Turn: {formatCurrency(cost)} | Context Use: {formatPercentage(percentageUsed)}</span
			>
		{:else}
				...
				<table>
					...
					<tbody>
						<tr>
							...
							<td class="px-2 text-right">{formatTokenCount(usage.input_tokens)}</td>
							{#if tokenInfo.pricing}
								<td class="px-2 text-right">{formatPrice(tokenInfo.pricing.input)}</td>
								<td class="pl-2 text-right"
									>{formatTableCost((usage.input_tokens * tokenInfo.pricing.input) / 1_000_000)}</td
								>
							{/if}
						</tr>
						<tr>
							...
							{#if tokenInfo.pricing}
								...
								<td class="pl-2 text-right"
									>{formatTableCost(
										(usage.output_tokens * tokenInfo.pricing.output) / 1_000_000
									)}</td
								>
							{/if}
						</tr>
